...
import com.fh.util.QRCodeUtils;
	...
	public Object createTwoDimensionCode(){
		...
		if(null == encoderContent){
			...
			try {
				...
				byte[] imageByte = QRCodeUtils.getImageByte(encoderContent);
				FileUtil.getFileFromBytes(imageByte, filePath+".png");
				//old    TwoDimensionCode.encoderQRCode(encoderContent, filePath, "png");							//执行生成二维码
			} catch (Exception e) {
				errInfo = "error";
			}
		}
	...
	public Object readTwoDimensionCode(){
		...
		if(null == imgId){
			...
			try {
				...
				readContent = QRCodeUtils.decoderQRCodeFile(filePath);
				// old   readContent = TwoDimensionCode.decoderQRCode(filePath);//执行读取二维码
			} catch (Exception e) {
				errInfo = "error";
			}
		}
